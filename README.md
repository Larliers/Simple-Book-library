...
| **当前 main** | 在稳定版基础上增加独立标签管理、三类资源无刷新 Quick Add、Library/书籍合集持久化排序、多选与批量加入合集、总书库图片文件夹书籍导入、集合规则自动归档，以及后续缺陷修复 |
...
> **仅当前 main：** 总书库还能把根目录第 1～扫描深度层内、无子目录、至少含 3 张图片且图片多于其他文件的目录作为一本普通书籍导入。卡片显示 `IMG` 角标，首张自然序图片用于自动封面和双击打开；该能力尚未包含在 v2.4.0 稳定版中。
...
- 该能力只整理已经导入的本地资源，不扫描网络或下载内容；目前仅在 main 源码中提供，尚未包含在 v2.4.0 稳定版。
