...
| **Comic** | 含图片的**叶子文件夹**（jpg / jpeg / png / webp） | 按文件夹一本浏览封面；标题取文件夹名；可选瀑布流或分页 |
...
- 漫画（**预期设计**）：面向「以图片存盘、文件夹为一本」的读者。扫描把含 jpg/jpeg/png/webp 的**叶子文件夹**识别成一本，标题用文件夹名；会先快速占位封面，再后台生成压缩缩略图。**不支持** CBZ / CBR / ZIP 等压缩包漫画。
