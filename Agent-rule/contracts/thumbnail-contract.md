...
- 保证 Library `.imgfolder` 的自然序首图写入 `cover_image_path`，并生成 420×620 边界内的首帧 WebP 到 `book/compressed`；解码失败返回结构化扫描 warning，资源仍可入库。
...
- Settings 的 Library `cleanup/regenerate` 同样支持 `.imgfolder`；`regenerate` 保留文件仍有效的 `manual` 缩略图，否则只读取数据库保存的 `cover_image_path` 重建，不重新遍历图片书目录。
