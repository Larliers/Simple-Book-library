...
## 对外文档维护
- 根目录 `README.md` 与 `README.en.md` 必须同步维护产品定位、稳定版能力、main 分支状态、限制和用户入口；两种语言保持事实一致，但不要求逐句直译。
- 版本、下载和功能声明必须先对照 `src/bookhub/version.py`、Git tag / Release 与当前源码；尚未发布的 main 能力必须与稳定版明确分开。
- 两份 README 必须各保留一个 `**vX.Y.Z**` 粗体版本标记，供 `.github/workflows/release.yml` 自动更新。
- README 主截图必须来自真实运行时，使用匿名或虚构数据，不得暴露用户书库、私人路径或未授权封面；标准桌面主图固定为 1920×1080。
- README 文案以用户任务和真实搜索意图组织，关键词自然出现，不使用关键词堆砌、未经验证的性能数据或超出项目边界的能力承诺。
