...
## 2026-09-22 - 集合规则独立交互原型

### 任务
在正式实现搁置期间，先提供可双击检查的单文件 HTML 原型，用虚构数据确认合集右键入口、设置页统一管理和规则编辑流程。

### 实现内容
- 根目录新增 `collection-rules-prototype.html`，全部 CSS/JavaScript 内联，不依赖网络、运行时 Bridge、数据库或真实书库。
- 使用书籍、Text Novel、Comic 三类虚构合集，模拟合集右键“编辑集合规则…”与“设置 → 集合规则”双入口，两处复用同一规则状态和编辑器语义。
- 条件编辑支持全部/任一、包含/不包含/开头/结尾/完全等于和逐条件大小写开关；模拟影响预览、关闭规则两种处置、手动排除与取消排除。
- 增加 Glass/Vaporwave 和桌面/740px/600px 预览切换；窄屏将规则列表与编辑器纵向排列，600px 主导航只保留图标。
- 仅同步 `src_construction.md`、worklog、decision-log 与当日 history；正式 UI、Repository、Bridge、扫描、contracts、README 均未修改。

### 影响、兼容性与回滚
- 原型位于项目根目录且不被应用加载；直接双击即可检查，刷新页面恢复初始演示数据。
- 不新增依赖、配置、持久化或 schema，不能作为已实现产品能力对外声明。
- 回滚只需移除该 HTML 及本次文档留档，不影响运行时和用户数据。
