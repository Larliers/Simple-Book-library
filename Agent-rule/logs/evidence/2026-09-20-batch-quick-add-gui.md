...
- 右键未选中资源会把既有批量选区收敛为该资源单选；右键已选中资源仍保留整批。
- 离开标签/合集详情、切页、搜索、排序、刷新或进入不可多选视图时，同时清空选中键与有序数据范围，`Ctrl+A` 不会重新选中隐藏资源。
- 提交期间统一锁定弹窗内全部按钮、输入框、选择框和动态 chip；失败回调后全部恢复可操作状态。
- 卡片多选容器使用 `listbox`/`option`，列表使用 `grid`，均声明 `aria-multiselectable=true`；批量标签与合集搜索输入均有显式 `label[for]`。
- 审查修复后重新执行 Glass 与规范值 `vaporwave` 的全部视口、成功/失败路径，结果与上文一致。
