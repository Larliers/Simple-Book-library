...
- `src/tests/test_web_bridge_smoke.py`：Web Bridge / scheme / Text Rules CRUD 冒烟；`NAV_ITEMS` 含图书馆/书籍合集/文本小说/小说合集/漫画/漫画合集/随机推荐；验证 `getRandomRecommendations()` 三类来源隔离、每列最多 3 项、数量不足、无重复与缺失排除，并调用 Node 行为脚本验证前端三类 `sourcePage` 路由、会话缓存、整组重抽、防重复请求、搜索恢复、显式失效及旧响应淘汰；bootstrap settings 含 `perRootScanStrategyEnabled`、`comicScanStrategy`、`appVersion` 与目录策略 i18n keys；library payload item 含 `extension`；`createCollection(page,name)` / `getCollections(page)`；`setRootScanStrategy` / `checkForUpdates` / `openExternalUrl` slot 存在性；`openResource` 对 CBZ/文件夹漫画解析首张图片路径（mock `_open_external`）。
- `src/tests/js/test_random_recommendations.js`：以 Node 内置 `vm` 和最小 DOM 假件真实执行生产 `app.js`；覆盖图书/小说/漫画卡的单击、键盘、双击、右键及详情按钮来源路由，缓存/重新推荐/防重复、搜索禁用后恢复、候选源失效和 pending 旧回调淘汰。
...
- `src/bookhub/i18n/locales/zh-cn.json`：中文文案键值表；含随机推荐入口、三类列标题、重新推荐、加载/空列提示；设置导航将路径与扫描合并命名；含缩略图缓存目录与 Fast 指纹漏检风险提示。
...
- `src/bookhub/library/thumbnail_tasks.py`：缩略图清理与重建任务实现；cleanup 仅 `unlink` `preview_dir` 内路径；漫画 `cover_fingerprint` 以 `manual:` 开头时跳过 regenerate，保留用户手选封面；`resolve_comic_open_path()` 供 UI 外部打开漫画时解析可读路径（文件夹漫画返回源图，CBZ 经 `prepare_cbz_for_external_viewer` 将全部页解压到 `preview/comic/read/` 后打开第一页供系统看图软件浏览）。
- `src/bookhub/library/thumbnail_worker.py`：缩略图任务线程包装；构造时可注入 `preview_dir`。
...
- `src/bookhub/ui/web_window.py`：当前主窗口 `WebAppWindow`；`set_web_page_background(skin, theme)` 按 skin×theme 同步底色；`web_zoom_factor` 启动 `setZoomFactor` 恢复并以轮询+debounce 写回 `app_settings`；负责后端编排——扫描/缩略图 worker（传入 `preview_dir`）、`PreviewCacheMigrateWorker` 换缓存目录、`UpdateCheckWorker` 检查 GitHub 最新 Release 并通过 `updateCheckResult` 回传前端、原生 `QFileDialog` 添加根目录/选缓存目录与编辑封面（写入 `repo.preview_dir`；漫画写 `manual:` fingerprint）、`remove_from_library`（仅删库记录不删磁盘）、`open_text_rules` 转交 Bridge 打开 Web 面板、字体与设置写库（含 `text_encoding_preference`、`perRootScanStrategyEnabled`、`comicScanStrategy`）；新增/移除资源根、删除资源及扫描完成时以 `recommendations_invalidated=True` 通知推荐候选源变化，封面/缩略图等元数据更新不触发重抽；扫描完成 Toast 计入 `comic_added_count`，冲突日志优先写 `incoming_path`；忙时再点扫描/缩略图/换缓存/检查更新会 Toast，并通过 `scanState.kind` 推送忙碌态。
- `src/bookhub/ui/web/js/app.js`：单一 SPA 壳；新增 `random_recommendations` 页面，会话内缓存 `State.recommendations`，首次进入或收到显式候选源失效标记时调用 `getRandomRecommendations()`，并以 `recommendationRequestId` 淘汰失效前的异步响应；标签、收藏和合集刷新不重抽；页面头部可整组重新推荐；`renderRecommendations` 固定渲染图书/小说/漫画三列且每列纵向最多 3 张封面，推荐卡以列级 `sourcePage` 路由详情、双击打开、右键与 Quick Add；推荐页禁用搜索并隐藏视图切换。其余职责包括：`loadSkinStylesheets`/`setUiSkin` UI 皮肤（Glass/Vaporwave）切换与 vw-scene 挂载；主内容区封面网格/合集/表格/漫画瀑布流与分页窗口化；三类合集、搜索建议、设置页、Quick Add/合集模态、日/夜主题引擎、扫描任务与格式角标等交互。
...
- `src/bookhub/ui/web_bridge.py`：`UiBridge(QObject)` 前后端桥；新增 `PAGE_RANDOM_RECOMMENDATIONS` 导航和只读 `getRandomRecommendations()` Slot，从未缺失且未受搜索过滤的 Library/Text Novel/Comic 来源分别无重复抽取最多 3 项，并返回列级带 `sourcePage` 的固定三列 JSON；`push_resources(recommendations_invalidated=False)` 用显式字段区分普通页面刷新与推荐候选源变化。其余职责包括 UI 皮肤与主题、设置/扫描/更新、资源与合集 CRUD、按页搜索、详情与外部打开、Text Rules，以及 `resourcesChanged` 等信号推送。
- `src/bookhub/library/repository.py`：`PRAGMA foreign_keys` + `busy_timeout`；删书/漫画与移根时清关联表（含 `collection_comics`）；启动 orphan 清理；`collections.kind`（book/text_novel/comic）+ `collection_comics`；跨类加入拒绝；既有合集默认 book 并剥离小说成员；`favorite_*` 一次性迁入名为「收藏」的对应 kind 合集（表保留不 DROP）；`hash_strategy` 缺省与非法值回退均为 `quick`；`comic_view_mode` 缺省为 `pagination`；`viewport_buffer_screens` 缺省 3（允许 3–6）；`grid_columns` 缺省 6（允许 4/5/6/7/8/10/12，限制每行封面数）；`comic_title_conflict_policy` 缺省 `skip_incoming`；`text_encoding_preference` 缺省 `simplified`。
...
- `src/bookhub/ui/web/css/base.css`：布局/结构/动画（无 skin 色板）；Glass 与 Vaporwave 共用；1120px 断点以 `.app-shell > .detail-panel` 确保响应式详情栏落位不被后加载皮肤覆盖。
...
- `src/bookhub/ui/web/css/skins/glass/components.css`：玻璃拟态组件样式；含随机推荐三列/纵向封面栈及键盘焦点样式、`.cover-wrap`/`.format-badge` 书籍格式角标、`.progress.busy span` 条纹不定进度动画。
...
- `src/bookhub/ui/web/css/skins/vaporwave/components.css`：蒸汽波组件重皮肤；与 Glass 保持随机推荐三列/纵向封面栈和键盘焦点布局一致，并使用霓虹描边、硬阴影与 Space Mono；其余组件选择器继续与 Glass 对齐，零 backdrop-filter。
...
## 4. 当前关键实现（简要）
- 2026-09-09 随机推荐页：侧栏在漫画合集后新增 `random_recommendations`；Bridge 的 `getRandomRecommendations()` 从未缺失且不受搜索条件影响的图书/文本小说/漫画三类来源各自无重复抽取最多 3 项；前端会话缓存推荐结果，固定三列纵向封面布局，支持整组重新推荐，并通过列级 `sourcePage` 复用正确的详情、双击打开、右键与 Quick Add 交互；仅扫描/资源增删显式使缓存失效，request id 阻止旧响应写回；Glass/Vaporwave 双皮肤同步。
