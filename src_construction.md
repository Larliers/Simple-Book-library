...
- `src/bookhub/library/formats/`：新格式解析与封面提取；`registry.py` 是 Library 格式支持、元数据提取与缩略图生成的单一注册入口（延迟导入）；其余 `html_md`/`fb2`/`docx_fmt`/`cbz`/`zip_safety`/`common` 提供具体实现。Library 用内嵌图或标题占位卡；Comic CBZ 取包内首图；docx/fb2.zip/cbz 经成员数与未压缩体积上限防 zip bomb。
...
- `src/bookhub/library/thumbnail_tasks.py`：缩略图清理与重建任务实现；cleanup 仅 `unlink` `preview_dir` 内路径；漫画 `cover_fingerprint` 以 `manual:` 开头时跳过 regenerate，保留用户手选封面。
