...
- `src/bookhub/library/data_paths.py`：缩略图缓存目录解析；默认 `PROJECT_ROOT/img_preview`；空/相对/不可写路径回退默认；`preview_cache` 模式枚举。
